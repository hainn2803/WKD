...
def wkd_feature_loss_with_interbatch_multilevel(f_s, f_t, eps=1e-5, grid=1):
    if grid == 1:
        f_s_avg, f_t_avg = f_s.mean(dim=(-1,-2)), f_t.mean(dim=(-1,-2))
        f_s_std, f_t_std = torch.sqrt(f_s.var(dim=(-1,-2)) + eps), torch.sqrt(f_t.var(dim=(-1,-2)) + eps)
        mean_loss = F.mse_loss(f_s_avg, f_t_avg, reduction='sum') / f_s.size(0)
        ...
        f_s_avg = f_s_avg.reshape(batch_size, dim, -1)
        f_t_avg = f_t_avg.reshape(batch_size, dim, -1)
        f_s_std = f_s_std.reshape(batch_size, dim, -1)
        f_t_std = f_t_std.reshape(batch_size, dim, -1)

        interbatch_cost = inter_batch_loss_multilevel_gaussian(mu1s=f_s_avg, Sigma1s=f_s_std**2, mu2s=f_t_avg, Sigma2s=f_t_std**2)
...
class WKD(Distiller):
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        if self.enable_wkdf:
            ...
            mean_loss, cov_loss, interbatch_cost = wkd_feature_loss_with_interbatch_multilevel(f_s, f_t, self.eps, grid=self.spatial_grid)
